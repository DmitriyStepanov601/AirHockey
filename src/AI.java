...
/**
 * A class that describes the way the AI works
...
public class AI {
     ...
     * @param game - game
     * @see AI#AI(float,float,int,AirHockey)
     */
    public AI(float x, float y, int difficulty, AirHockey game) {
        this.x = x;
        this.y = y;
        this.radius = 15;
        this.velx = 0;
        ...
        this.game = game;
        this.midline = ((float) game.getHeight() / 2) - 30;
    }
    ...
    public void update() {
        ...
        if (difficulty == 0) {
            if (game.getPanel().getPuck().getY() < midline)
                ...
                moveTo((float)game.getWidth() / 2, 50);
        } else if (difficulty == 1) {
            float middle = ((float)game.getWidth() / 2) - 16;
            if (game.getPanel().getPuck().getX() > (middle - 100) &&
                    game.getPanel().getPuck().getX() < (middle + 100)) {
                moveTo(middle, 50);
            ...
            } else if (game.getPanel().getPuck().getVel() < .5
                    && game.getPanel().getPuck().getY() < midline) {
                moveTo(game.getPanel().getPuck().getX(), game.getPanel().getPuck().getY());
            } else {
                moveTo((float)game.getWidth() / 2, 50);
            }
    ...
    public void moveTo(float x, float y) {
        ...
        velx = (float)(l.getEndX() - l.getStartX()) / 20;
        vely = (float)(l.getEndY() - l.getStartY()) / 20;
    }
